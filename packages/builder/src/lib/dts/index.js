...
class DtsWriter extends events_1.default {
    ...
    resolveModuleDefault(moduleId) {
        const resolvedId = moduleId.replace(/^.+\/src/, 'src');
        const removedIndexId = resolvedId.replace(/\/index$/, '');
        if (this.mainId !== this.prefixModule(resolvedId) &&
            removedIndexId !== this.options.name) {
    ...
    resolveMainId() {
        var _a;
        return `${this.options.name}/${((_a = this.options.main) === null || _a === void 0 ? void 0 : _a.replace(/^\/+/, '')) || 'index'}`;
    }
    prefixModule(moduleId, isExternal = false) {
